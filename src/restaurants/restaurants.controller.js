...
async function listAverageRatingByOwner(req, res, next) {
  // your solution here
  try{
  const listAvgRatingdata = await service.listAverageRatingByOwner();
  console.log("--**********-- ", listAvgRatingdata.length, ' :::det::: ', listAvgRatingdata)
  // console.log(":::::: ", listAvgRatingdata.map((item)=>{return{ ...item,"avg": Number(item.avg)}}));
  console.log(":::::: ", listAvgRatingdata.map((item)=>{return{"avg": Number(item.avg), "owner_name":item.owner_name}}));

  // res.json({ data: listAvgRatingdata.map((item)=> {
  //    return {"avg": Number(item.avg), "owner_name":item.owner_name}
  // }) });
...
async function averageRating(req, res, next) {
  ...
  // console.log("averageRating:::data::: ", data);
